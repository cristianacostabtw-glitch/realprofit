...
def _meli_etiquetas_procesar(data):
    ...
    for d in labels:
        bb = d["bbox"]
        bw, bh = max(1.0, bb.width), max(1.0, bb.height)
        ...
        page.show_pdf_page(dest, doc, d["page"], clip=bb)
        sku_fmt = "X%d %s" % (d["potes"], "POTE" if d["potes"] == 1 else "POTES")
        detalle.append({"sku": sku_fmt})
        orders.append({"tracking": d["trak"], "buyer": d["buyer"],
                       "sku_meli": d["sku"], "potes": d["potes"]})
    if detalle:
        _sku_hoja_empaquetar(out, detalle)          # hoja final PARA EMPAQUETAR
    buf = io.BytesIO()
...
def _seg_mapa_orders(store, hdr, numeros) -> dict:
    """number(str) → order dict de TiendaNube. Cada página de TN tarda ~6s y trae 200 (per_page=200).
...
def meli_duplicar():
    ...
    try:   # cantidad: si no la mandan, se copia la del original (se duplica todo menos título/precio/cuotas)
        qty = int(d.get("stock")) if d.get("stock") not in (None, "") else int(s.get("available_quantity") or 1)
    except Exception:
        qty = int(s.get("available_quantity") or 1)
    try:
...
def _bpu_from_sku(sku):
    """Botellas de 30 ml por unidad, sacadas del SKU (lo confiable). 0 = no detectado."""
    s = (sku or "").lower().strip()
    if not s:
        return 0
    m = re.search(r"x\s?([1-9])", s)
    if m:
        return int(m.group(1))
    m = re.search(r"([1-9])\s*pote", s)
    if m:
        return int(m.group(1))
    m = re.fullmatch(r"\s*([1-9])\s*", s)
    if m:
        return int(m.group(1))
    return 0
...
function bpuFromSku(s){ s=(s||'').toLowerCase().trim(); if(!s)return 0; var m=s.match(/x\s?([1-9])/); if(m)return +m[1]; m=s.match(/([1-9])\s*pote/); if(m)return +m[1]; m=s.match(/^\s*([1-9])\s*$/); if(m)return +m[1]; return 0; }
function bpuFor(it){ return STK30.bpu[it.id] || bpuFromSku(it.sku) || bpuAuto(it.title) || 1; }
...
function renderStock30(){ var box=document.getElementById('mlc'); if(!box)return; var v=STK30.items; var s30=(STK30.stock30!=null?STK30.stock30:'');
 ...
 var rows=v.map(function(it){ var k=bpuFor(it); var units=(STK30.stock30!=null?Math.floor(STK30.stock30/Math.max(1,k)):'—');
...
def _resolver_suc_completo(suc_raw, lat, lng, pid, cp, sucs):
    ...
    try:
        lv = _and_suc_live(suc_raw, cp or "")
    except Exception:
        lv = None
    if lv:
        return lv, "vivo"
...
function boot(){ get('/wa-estado').then(function(s){ EST=s; renderApp(); if(s&&s.conectado){ unlock(); return; }
  ...
  get('/wa-web-status').then(function(w){ if(w && (w.ok===false || w.status==='connected')){ waTab('web'); } else { lockScreen(); } }).catch(function(){ lockScreen(); });
}); }
