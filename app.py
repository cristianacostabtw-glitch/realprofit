...
(function(){
 function strip(){
  try{
   ...
   for(var i=0;i<kids.length;i++){ var ch=kids[i]; if(ch.id && ch.id.indexOf('rp-meli-sub-')===0) continue; ch.style.display = (ch.querySelector('a[href="/dashboard"]')||ch.id==='rp-prod-nav'||ch.id==='rp-comis-nav'||ch.id==='rp-desp-nav'||ch.id==='rp-fact-nav'||ch.id==='rp-mov-nav'||ch.id==='rp-ads-nav'||ch.id==='rp-wa-nav'||ch.id==='rp-meli-nav'||ch.id==='rp-stock-nav') ? '' : 'none'; }
   ...
   if(!nav.querySelector('#rp-meli-nav')){
    ...
    if(ml0){ var cml=ml0.cloneNode(true); cml.id='rp-meli-nav'; cml.style.display='';
     ...
     // Sub-ítems (acordeón) DENTRO de la barra, ocultos hasta tocar MercadoLibre.
     var MLSUB=[['ventas','Ventas','🧾'],['mensajes','Preguntas','💬'],['envios','Envíos','📦'],['sku','SKU','🏷️'],['stock','Stock','📊'],['metricas','Métricas','⭐']];
     var prevN=cml;
     MLSUB.forEach(function(sb){ var cs=cml.cloneNode(true); cs.id='rp-meli-sub-'+sb[0]; cs.className=(cs.className||'')+' rp-meli-sub'; cs.style.display='none';
       var asub=cs.querySelector('a'); if(asub){ var nsub=asub.cloneNode(true); asub.parentNode.replaceChild(nsub,asub); nsub.addEventListener('click',(function(k){return function(e){e.preventDefault();e.stopPropagation();try{window.rpMeliOpen(k);}catch(err){}};})(sb[0])); }
       var icc=cs.querySelector('.material-symbols-outlined'); if(icc){ icc.style.fontSize=''; icc.textContent=''; icc.innerHTML='<span style="font-size:16px;line-height:1">'+sb[2]+'</span>'; }
       var spc=cs.querySelectorAll('span'); for(var q=0;q<spc.length;q++){ var s9=spc[q]; if(!s9.classList.contains('material-symbols-outlined') && s9.children.length===0){ s9.textContent=sb[1]; try{s9.style.fontSize='13px';s9.style.opacity='.9';}catch(e){} } }
       prevN.parentNode.insertBefore(cs, prevN.nextSibling); prevN=cs; });
    }
   }
...
(function(){
 ...
 window.rpMeli=function(open){ var o=document.getElementById('rp-meli-ov'); if(!o)return;
   ...
   o.style.display=open?'block':'none'; };
 window.rpMeliToggle=function(){ var subs=document.querySelectorAll('.rp-meli-sub'); var show=(subs.length && subs[0].style.display==='none');
   for(var i=0;i<subs.length;i++) subs[i].style.display=show?'':'none';
   if(show) window.rpMeliOpen('ventas'); };
 window.rpMeliOpen=function(k){ var f=document.getElementById('rp-meli-frame'); if(f){ f.setAttribute('src','/meli?f='+encodeURIComponent(k)); } window.rpMeli(true);
   var subs=document.querySelectorAll('.rp-meli-sub'); for(var i=0;i<subs.length;i++){ var aa=subs[i].querySelector('a'); if(aa) aa.style.background=(subs[i].id==='rp-meli-sub-'+k)?'rgba(255,230,0,.14)':''; } };
...
def pf_version():
    ...
    return jsonify({"ok": True, "v": "2026-08-24-meli-acordeon-barra"})
...
def _and_cfg(wb):
    if _AND_CFG["cpidx"] is None:
        ...
        _AND_CFG["norm"] = {_and_norm(o): o for o in sucs}   # índice para match EXACTO
    return _AND_CFG["cpidx"], _AND_CFG["sucs"]


def _and_suc_exacto(zeny):
...
def _and_domicilio(calle, numero, extra):
    ...
    if not numero and not _num_sola:                 # 1) número pegado en la calle ("Grecia 1036")
        ...
        if n2:
            calle, numero = c2, n2
    # calle "débil" = placeholder ("Casa"), basura sin nombre NI número ("A"), o un número suelto de
    # 4+ dígitos (tipo CP, ej "8322", NO una calle). Calle numerada corta ("86 n", "70") es válida.
    calle_debil = (calle_ph
                   or (not _re_and.search(r"[A-Za-zÁÉÍÓÚÑáéíóúñ]{3,}", calle)
                       and not _re_and.search(r"\d", calle))
                   or bool(_re_and.fullmatch(r"\d{4,}", calle.strip())))
...
function boot(){ try{ var qp=new URLSearchParams(location.search); var f=qp.get('f'); if(f)SEL=f; }catch(e){}
 var sd=document.getElementById('side'); if(sd)sd.style.display='none';   // el submenú ahora vive en la barra lateral
 renderMain();
...
@app.get("/pf-diag-trk")
def pf_diag_trk():
    """TEMPORAL — cuál fue el último pedido con tracking (seguimiento) cargado en Shopify. Gateado."""
    ...
    for _ in range(20):
        try:
            r = requests.get("%s/orders.json" % base, headers=H, params={
                ...
                "fields": "id,order_number,name,fulfillments,cancelled_at"}, timeout=30)
        ...
        for o in lote:
            ...
            if trk:
                con_trk.append({"num": o.get("order_number"), "trk": trk})
    con_trk.sort(key=lambda x: x["num"] or 0)
...
def pf_diag_units():
    """TEMPORAL — {nº pedido → unidades (potes)} de un rango, para estampar el SKU. Gateado."""
    if (request.args.get("key") or "") != "chequeo-noxa-2026":
        return jsonify({"ok": False}), 403
    shopq = (request.args.get("shop") or "").strip().lower()
    lo = int(request.args.get("lo") or 0); hi = int(request.args.get("hi") or 99999)
    ...
    for _ in range(20):
        try:
            r = requests.get("%s/orders.json" % base, headers=H, params={
                "status": "any", "limit": 250, "since_id": since, "order": "id asc",
                "fields": "id,order_number,line_items,cancelled_at"}, timeout=30)
            lote = (r.json() or {}).get("orders", []) if r.status_code == 200 else []
        except Exception as e:
            return jsonify({"ok": False, "msg": str(e)[:100]})
        if not lote:
        ...
        for o in lote:
            ...
            if n < lo or n > hi:
                continue
            out[str(n)] = sum(int(li.get("quantity") or 0) for li in (o.get("line_items") or []))
    return jsonify({"ok": True, "n": len(out), "units": out})
